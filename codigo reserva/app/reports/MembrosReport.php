...
class MembrosReport
{
    ...
    public function gerar(array $filtros): string
    {
		...
		foreach ($mapAtivos as [$cell, $where]) {
            $sheet->setCellValue(
                $cell,
                $this->count("{$where}
				AND permanencia = 'Ativo'
				AND data_conversao <= :data_fim", [
                    ':data_fim' => $dataFim
                ])
            );
        }
        ...
        foreach ($mapPermanencia as [$cell, $where]) {
            $sheet->setCellValue(
                $cell,
                $this->count("{$where}
				AND data_conversao <= :data_fim", [
		...
		$sheet->setCellValue('E11', date('d/m/y'));

        // 6️⃣ Salvar arquivo
        $outputPath = BASE_PATH . '/storage/generatedReports/Contagem_de_Membros_e_convertidos_' . time() . '.xlsx';
